...
class UserService {
  ...
  Future<AppResponse<UserViewModel>> loadUserData(Map<String, dynamic> requestInformation) async {
    ...
    if (responseBody["code"] >= 200 && responseBody["code"] <= 300) {
      return AppResponse(
          ResponseGrades.SUCCESS, responseBody["code"], responseBody["message"], UserViewModel.fromJson(responseBody));
    }
    return AppResponse(ResponseGrades.ERROR, responseBody["code"], responseBody["message"]);
  }

  Future<AppResponse> saveProfileDetails(Map<String, dynamic> requestInformation) async {
    Response response = await userRepository.saveProfileDetails(requestInformation);
